...
def takeFirstPeak(peak_event:list, min_inter_train, max_intra_train, frequence):
    """Take the first pulse in a train, filter by maximum and minimum time between two pulse and two train
    return the index of the first pulse. For the algorythm to work the intra train space must be less than the
...
def plot_psth(t_inf, t_supp, psth_low, psth_mean, psth_high, nb_samp, ylimite:tuple,courant):
    ...
    plt.ylim(ylimite)
    plt.title(f'PSTH with n = {nb_samp} samples and current ={courant}')
    plt.xlabel('Time [s]')
